...
Promise.all([
    fetch("fl.json").then(res => res.json()),
    fetch("sl.json").then(res => res.json()),
    fetch("word.json").then(res => res.json())
])
    .then(([flData, slData, wordData]) => {
        fl = flData;
        sl = slData;
        word = wordData;

        // Populate letter1Select
        ...
        // Get selected value
        const letter1Selected = letter1Select.options[letter1Select.selectedIndex]?.text || "";

        // Populate letter2Select
        if (sl[letter1Selected]) {
        ...
        const letter2Selected = letter2Select.options[letter2Select.selectedIndex]?.text || "";

        // Populate wordSelect
        const firstTwoLetters = letter1Selected + letter2Selected;
        if (word[firstTwoLetters]) {
        ...
        const wordSelected = wordSelect.options[wordSelect.selectedIndex]?.text || "";
